...
def get_ignore_list(ignore_paths):
    return [line.strip() for path in ignore_paths if os.path.exists(path) for line in open(path, 'r')]
...
def process_repository(repo_path, ignore_spec, fp):
    for root, _, files in os.walk(repo_path):
        for file in files:
            ...
            if not should_ignore(relpath, ignore_spec):
...
def main():
    ...
    args = par.parse_args()
    ignore_list = get_ignore_list([os.path.join(args.repo_path, i) for i in args.ignore])
    ignore_spec = PathSpec.from_lines(GitWildMatchPattern, ignore_list)
